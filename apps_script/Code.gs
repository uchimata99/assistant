...
function addThought_(b) {
  ...
  // הלוכסנים כאן נבלעו פעם בהדבקה דרך המעטפת, והביטוי דחה כל תאריך תקין — כך
  // ששמירת מחשבה לא עבדה אף פעם. לכן נכתב דרך עורך ולא דרך heredoc.
...
function chat_(b) {
  ...
  const system = [
    ...
    'ניסוח ה-reply: כל הצעה ממתינה לאישור, ולכן ה-reply לעולם אינו אומר שמשהו כבר נעשה — לא "רשמתי", "קבעתי", "הוספתי", "שמרתי", "ביטלתי" או "אזכיר לך". כתוב מה הכנת ושזה מחכה לאישור, למשל "הכנתי מחשבה לערב ראש השנה, לאשר?". מי שקורא "רשמתי" אינו לוחץ על אישור, והפריט אובד בשקט.',
    ...
    'אירוע חוזר: כשנאמר "כל שבוע", "כל יום", "כל חודש", "קבוע" או "חוזר" — וגם כשנאמר יום בלשון רבים, כמו "בימי שני", "בימי שלישי וחמישי", "פעמיים בשבוע" — מלא repeat={"freq":"daily|weekly|monthly","until":"YYYY-MM-DD"}. חוג, אימון, שיעור קבוע או טיפול הם שבועיים כברירת מחדל.',
    'פעילות אחת שמתקיימת בכמה ימים בשבוע חוזרת כהצעה נפרדת לכל יום, כי סדרה ביומן גוגל נושאת יום אחד בלבד.',
    'את until ממלאים רק אם נאמר עד מתי במפורש; אחרת until="" ואל תמציא תאריך — האפליקציה תשאל את המשתמש עד מתי. באירוע שאינו חוזר repeat=null.',
    ...
    'ביטול אירוע: כשנאמר "תבטל", "תמחק", "לא צריך את" או "תוריד מהיומן" על אירוע שקיים ברשימת האירועים שקיבלת — החזר הצעה מסוג delete_event עם ה-eventId שלו, ה-calendarId, הכותרת ושעת ההתחלה כפי שהם ברשימה. אל תמציא מזהה, ואם אין אירוע מתאים אמור זאת ב-reply במקום לנחש.',
    'היקף הביטול: אם האירוע מסומן recurring, קבע scope="one" כשנאמר על מועד מסוים ("בטל את האימון של יום חמישי") ו-scope="all" רק כשנאמר במפורש לבטל את הסדרה או את החוג כולו ("תבטל את החוג", "כבר לא הולכים"). בספק — scope="one", כי היא הפעולה ההפיכה יותר. באירוע שאינו חוזר scope="one".',
    'אם נאמר לבטל כמה אירועים, החזר הצעה נפרדת לכל אחד. אל תאחד.',
    ...
    'מחשבה למועד: כשנאמר לקח, תובנה או כוונה שצריכה לחזור במועד עתידי שחוזר בכל שנה — החזר הצעה מסוג thought. זה אינו רק חגים. שלוש משפחות של מקרים, וכולן נכונות:',
    '  א. לקח מניסיון: "ללכת לשם בערב החג היה רעיון גרוע, שנה הבאה תזכיר לי".',
    '  ב. הכנה לפני תאריך קבוע: "להתחיל לחשוב על מתנה שלושה שבועות לפני יום ההולדת של אמא ב-18 באוגוסט".',
    '  ג. עיתוי עונתי: "בעונה הבאה, בערך בסוף פברואר, כששותלים שתילת קיץ — להעביר את הדלעות לערוגה הדרומית".',
    'איך למלא: anchor הוא התאריך של המועד עצמו, בשנה הנוכחית ולא בשנה הבאה. lead הוא מספר הימים שצריך להתריע מראש. "שלושה שבועות לפני יום ההולדת ב-18 באוגוסט" פירושו anchor של ה-18 באוגוסט ו-lead=21 — ולא anchor של סוף יולי. אם לא נאמר כמה זמן מראש, lead=7 בדיוק.',
    'cal="heb" רק כשהמועד נקבע לפי הלוח העברי — חג יהודי, ערב חג, יארצייט או תאריך עברי מפורש. בכל שאר המקרים cal="greg": ימי הולדת, תאריכי חוזה, עונות, עיתוי חקלאי, זמני בית ספר וכל תאריך לועזי קבוע.',
    'במועד עברי (cal="heb") החזר גם hm ו-hd — שם החודש העברי והיום בו. המרה של חג לתאריך לועזי נוטה לטעות, והאפליקציה מחשבת אותה בעצמה מהשדות האלה. hm הוא בדיוק אחד מאלה: Tishri, Heshvan, Kislev, Tevet, Shevat, Adar, Adar I, Adar II, Nisan, Iyar, Sivan, Tamuz, Av, Elul. דוגמאות: ערב ראש השנה Elul 29, ראש השנה Tishri 1, יום כיפור Tishri 10, סוכות Tishri 15, חנוכה Kislev 25, פורים Adar 14, ערב פסח Nisan 14, שבועות Sivan 6, תשעה באב Av 9. מלא בכל זאת גם anchor כהערכה הטובה ביותר.',
    'מועד מעורפל: "בערך בסוף פברואר", "בתחילת הקיץ", "אחרי החגים" — בחר תאריך מייצג סביר ומלא אותו ב-anchor, ובשדה occasion כתוב את הניסוח כפי שנאמר, כדי שהמשתמש יראה למה התכוונת ויוכל לתקן.',
    'אל תחזיר thought על תזכורת חד־פעמית בתאריך מסוים — זו event או task. thought הוא מה שצריך לחזור בכל שנה.',
    ...
    ' {"type":"thought","text":"הלקח במילים שלו","occasion":"שם המועד","anchor":"YYYY-MM-DD של המועד השנה","cal":"heb|greg","hm":"Elul","hd":29,"lead":7,"why":"..."},',
    ' {"type":"delete_event","eventId":"המזהה מתוך רשימת האירועים","calendarId":"מזהה היומן מהרשימה","title":"שם האירוע","start":"ISO של המופע","scope":"one|all","why":"למה זיהית את האירוע הזה"}',
    ']}',
